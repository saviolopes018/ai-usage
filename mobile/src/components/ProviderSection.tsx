...
function TokenConsumption({provider,theme}:{provider:ProviderUsage;theme:Theme}) {
  if(provider.provider!=='opencode'||!provider.tokens)return null;
  const periods=provider.tokens.periods;
  const values=[
    ['24 horas',periods?.['24h']?.totalTokens??provider.tokens.totalTokens],
    ['7 dias',periods?.['7d']?.totalTokens],
    ['14 dias',periods?.['14d']?.totalTokens],
    ['30 dias',periods?.['30d']?.totalTokens],
  ] as const;
  ...
  return <><View style={styles.tokenIntro}><Text style={[styles.tokenIntroTitle,{color:theme.colors.ink}]}>Consumo acumulado</Text><Text style={[styles.tokenIntroDetail,{color:theme.colors.muted}]}>O OpenCode informa uso de tokens, não limites de sessão.</Text></View><View style={styles.tokenGrid}>{values.map(([label,value])=><View key={label} accessible accessibilityLabel={`${label}: ${value===undefined?'sem dados':`${value} tokens`}`} style={[styles.tokenMetric,{backgroundColor:theme.colors.surface}]}><Text style={[styles.tokenPeriod,{color:theme.colors.muted}]}>{label}</Text><Text style={[styles.tokenTotal,{color:theme.colors.ink}]}>{value===undefined?'—':format(value)}</Text><Text style={[styles.tokenUnit,{color:theme.colors.muted}]}>tokens</Text></View>)}</View></>;
}
...
export function ProviderSection({ provider, theme, now, onRefresh, refreshing=false }: {provider:ProviderUsage;theme:Theme;now:number;onRefresh?:()=>void;refreshing?:boolean}) {
  const name=provider.provider==='codex'?'Codex':provider.provider==='claude'?'Claude':provider.provider==='opencode'?'OpenCode':provider.provider;
  const providerIcon=provider.provider==='codex'?require('../../assets/providers/codex.png'):provider.provider==='claude'?require('../../assets/providers/claude.png'):null;
  const providerColor=provider.provider==='claude'?theme.colors.claude:provider.provider==='opencode'?theme.colors.opencode:theme.colors.codex;
  const providerSoft=provider.provider==='claude'?theme.colors.claudeSoft:provider.provider==='opencode'?theme.colors.opencodeSoft:theme.colors.codexSoft;
